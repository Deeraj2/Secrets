...
const initialState = { name:  '', email: '', password: '', confirmPassword: '', pic: '' };
...
const Authentication = () => {
  ...
  const signUp =async()=>{
    if(!formData.name || !formData.email || !formData.password ||  !formData.pic || !formData.confirmPassword ){
        setAlert({
        open: true,
      ...
      try {
        ...
        setAlert({
          open: true,
          message: `Signup In Successful. Welcome ${user?.name}`
      });
      } catch (error) {
        setAlert({
        open: true,
  ...
  const signIn = async() => {
    ...
    try {
      const config = {
        ...
        setAlert({
          open: true,
          message: `Login In Successful. Welcome ${user?.name}`
      });
    } catch (error) {
      console.log(error);
      ...
      <div className='auth-gradient' />    
          <div className='auth-form' >
            <form className='auth-login' onSubmit={handleSubmit}>
              ...
              {isSignUp && <FileBase  type="file" name='pic'  multiple={false} onDone={({base64})=> setFormData({...formData, pic: base64})} />}
